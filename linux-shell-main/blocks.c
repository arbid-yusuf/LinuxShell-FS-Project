...
void blocks_init(const char *image_path) {
  ...
  bitmap_put(bbm, 0, 1); //bitmap is here
  bitmap_put(bbm, 1, 1); //claim block 1 and 2 for inode table
  bitmap_put(bbm, 2, 1);
  //block 0 for bitmap, block 1,2 for inode table, 3 and 4 for direct structs. We support up to 120 direct structs (120 files max), so we meet the requirements for the basic FS. We don't support much directory functionality (no files in directories or tiers of directories), but you can create them and rename them, so some partial credit would be appreciated :). No big files allowed.
}
